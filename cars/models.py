...
class CarModel(models.Model):
    ...
    brand = models.ForeignKey(CarBrand, on_delete=models.CASCADE, related_name='models')  # Бренд, до якого належить модель
...
class Car(models.Model):
    ...
    price_per_minute = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'))  # Ціна оренди за хвилину
    
    # Технічні характеристики
    engine_capacity = models.DecimalField(max_digits=4, decimal_places=1, null=True, blank=True)  # Об'єм двигуна у літрах
    power = models.PositiveIntegerField(null=True, blank=True)  # Потужність двигуна у кінських силах
    seats = models.PositiveSmallIntegerField(validators=[MinValueValidator(1), MaxValueValidator(10)])  # Кількість місць у автомобілі
...
class CarReview(models.Model):
    ...
    car = models.ForeignKey(Car, on_delete=models.CASCADE, related_name='reviews')  # Автомобіль, до якого належить відгук
    user = models.ForeignKey('users.User', on_delete=models.CASCADE)  # Користувач, який залишив відгук
    rating = models.PositiveSmallIntegerField(validators=[MinValueValidator(1), MaxValueValidator(5)])  # Рейтинг автомобіля (від 1 до 5)
