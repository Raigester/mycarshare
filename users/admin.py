...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_verified_driver', 'is_staff')  # Поля для відображення у списку
    fieldsets = UserAdmin.fieldsets + (
        ('Додаткова інформація', {'fields': ('phone_number', 'date_of_birth', 'profile_picture',
                                             'driver_license_number', 'driver_license_expiry',
                                             'is_verified_driver', 'rating', 'is_blocked')}),  # Додаткові поля для редагування
    )
